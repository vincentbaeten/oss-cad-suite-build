...
def executeBuild(target, arch, prefix, build_dir, output_dir, nproc, pack_sources):
	...
	for i, j in env.items():
		if i.endswith('_DIR'):
			...
			params += ['-e', '{}={}'.format(i, j)]
	params += [
		'ghcr.io/vincentbaeten/oss-cad-suite-build/cross-'+ arch + ':1.1',
...
def generateYaml(target, build_arch, write_to_file):
	...
	BUCKET_URL = "https://github.com/vincentbaeten/oss-cad-suite-build/releases/download/bucket"
	for t in build_order:
		...
		yaml_content +="        with:\n"
		yaml_content +="          repository: 'vincentbaeten/oss-cad-suite-build'\n"
		if not target.top_package:
